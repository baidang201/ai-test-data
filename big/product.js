...
for (let i =0; i< 200; i++) {
  ...
  for (let index = 0; index < times; index++) {
    ...
    let block_diff = GetRandomNum(30,100);
    base_block += block_diff;
  ...
  const to_mnemonic = Util.mnemonicGenerate();
  const pair_to = keyring.addFromUri(to_mnemonic, { name: 'first pair' }, 'ed25519');
  let hash = CryptoJS.SHA256(pair_to.address + pair_to.address);

  let block_diff = GetRandomNum(30,100);
  base_block += block_diff;

  let warn_multiple = GetRandomNum(6,1000);
  ...
  arr.push({
    ...
    from: from,
    to: pair_to.address,
    balance: base * warn_multiple,
    flag: true
  })
}
//console.log(arr);
fs.writeFile("big-product.json", JSON.stringify(arr), 'utf8', function (err) {
  if (err) {
